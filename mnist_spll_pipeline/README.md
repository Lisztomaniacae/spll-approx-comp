...
Checkpoint-transfer approximate runs load the per-seed exact step checkpoint at each aggregate anchor step and reuse the same deterministic sum-case interval `s_A+1..s_B` that the aggregate exact curve used between anchors. For real posterior-threshold anchors, the continuation may stop early once its own full-window rolling true-sum posterior reaches the next exact checkpoint's displayed rolling posterior value. The synthetic final anchor is only a budget endpoint and must not trigger early stopping. In trajectory figures, exact aggregate posterior checkpoints are shown as distinct purple X markers on the displayed exact curve, and the green curve is drawn as separate pieces so segment restarts are visible.
