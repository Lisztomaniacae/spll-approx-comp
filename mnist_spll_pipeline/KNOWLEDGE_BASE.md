...
- Thesis figures are exported as 300 dpi PNGs and should be sized for A4 page-width embedding; `plot_palette.FIGURE_DPI` is the single source of truth for raster export resolution. Pipeline II combined exact-vs-approximation trace figures (`*_exact_vs_approx_combined.png`) should use no gray subtitle under the title and no legend title. The line widths are intentionally slim: about 1.05 for the pure exact line and 1.0 for pure approximate and approximate-continuation lines. Loss panels must use data-driven logarithmic y-limits with padding instead of fixed limits, otherwise the final low-loss checkpoint or continuation can be clipped. Do not thicken these curves unless the rendered output is checked against the thesis figures.
