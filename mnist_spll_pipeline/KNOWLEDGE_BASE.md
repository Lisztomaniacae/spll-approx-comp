...
| `read_mnist_lookup_calls` | Number of generated `readMNist` invocations during full-posterior enumeration. This counts lookup requests even when the active policy serves them from a per-measurement table. |
| `read_mnist_model_evaluations` | Number of underlying MNIST-model evaluations for the full-posterior measurement: cache misses for uncached/run-scoped execution, or unique-image precomputations for `precomputed_per_measurement`. |
| `true_candidate_read_mnist_lookup_calls` | Generated `readMNist` invocations during the separately measured true-candidate query. |
| `true_candidate_read_mnist_model_evaluations` | Underlying MNIST-model evaluations during the separately measured true-candidate query. |
...
- `mnist_lookup_accuracy_tradeoff_by_terms.png` uses the same model-row/term-column layout, but the blue axis shows the mean number of generated `readMNist` calls for full-posterior enumeration and the dashed blue line shows the exact lookup-count baseline in each panel. The red axis shows accuracy retained versus exact. The plot includes cutoff `0.0` because it is an approximate-code-path setting and a useful no-pruning lookup baseline.
...
- Thesis figures are exported as 300 dpi PNGs at the TU report text-block width; `plot_palette.FIGURE_DPI` remains the single source of truth for raster export resolution and the shared thesis dimensions live in `plot_palette.py`. Three-condition figures use a compact 2x2 layout with the spare panel available for legends. Reader-facing plots omit global titles because the thesis caption and surrounding section provide that context. Pipeline II combined exact-vs-approximation trace figures (`*_exact_vs_approx_combined.png`) use concise canonical labels for exact, approximation from initialization, exact milestone anchors, and checkpoint transfer. Loss panels must use data-driven logarithmic y-limits with padding instead of fixed limits so low-loss milestones and continuation segments are not clipped.
...
9. **Lookup calls are not always model evaluations.** Under `precomputed_per_measurement`, `read_mnist_lookup_calls` counts generated accesses to the fresh lookup table, while `read_mnist_model_evaluations` remains the number of unique images precomputed before timing. Use the former to study compiler/inference demand and the latter to discuss neural compute.
...
This reruns only the green checkpoint-to-checkpoint approximate continuations. It rebuilds aggregate exact checkpoints if needed, loads the exact per-seed step checkpoint at aggregate step `s_A`, and trains the approximate mode on the complete deterministic training interval `s_A+1..s_B` used by the exact segment. Crossing the next exact checkpoint's displayed rolling true-sum posterior is retained as diagnostic metadata only; it never truncates the segment. This fixed-interval rule ensures every green continuation reaches the next anchor's training iteration even when its posterior overshoots the target value early.
...
- Pipeline II combined comparison figures use a shared figure-level legend, no global plot title, no legend title, and concise canonical reader-facing labels. All Pipeline I and Pipeline II PNG figures use the shared 300 dpi export setting and TU report text-block dimensions; common three-panel comparisons use 2x2 layouts for readable thesis embedding.
