...
This reruns only the green checkpoint-to-checkpoint approximate continuations. It rebuilds aggregate exact checkpoints if needed, loads the exact per-seed step checkpoint at aggregate step `s_A`, and trains the approximate mode on the same deterministic training cases `s_A+1..s_B` used by the exact segment. For real posterior-threshold anchors it may stop early once the approximate full-window rolling true-sum posterior reaches the next exact checkpoint's displayed rolling posterior value. The synthetic final anchor is only a budget endpoint and must not trigger early stopping.
