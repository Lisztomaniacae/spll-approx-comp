...
from pipeline1_plotting import (
    combine_vertical_figure_assets,
    heatmap_specs,
...
def run_visualization_stage(config: Dict[str, Any]) -> None:
    ...
    figure_root = ensure_dir(
        resolve_path(config, (config.get("paths", {}) or {}).get("figures_root", "./figures"))
    )
    ...
    plot_predictive_effects_combined(
        ...
        term_counts=term_counts,
        threshold_order=threshold_order,
        output_path=main_dir / "predictive_effects_by_terms.png",
    )
    plot_true_candidate_metric_vs_cutoff(
        summary_rows=standard_rows,
        term_counts=term_counts,
        ...
        ylabel="Median true-sum-only runtime (ms)",
    )
    plot_true_candidate_metric_vs_cutoff(
        summary_rows=standard_rows,
        term_counts=term_counts,
    ...
    plot_true_candidate_metric_vs_cutoff(
        ...
        ylabel="True-sum survival fraction",
        ylim=(0.0, 1.05),
    )
    plot_true_candidate_metric_vs_cutoff(
        summary_rows=standard_rows,
        ...
        ylabel="Normalized true-sum probability",
        ylim=(0.0, 1.05),
        shared_ylabel=True,
    )
    if biased_tradeoff_rows:
        appendix_true_probability_row = appendix_dir / "true_candidate_probability_row_by_terms.png"
        plot_true_candidate_metric_row(
            summary_rows=standard_rows,
            term_counts=term_counts,
            threshold_order=threshold_order,
            output_path=appendix_true_probability_row,
            metric_key="mean_true_candidate_normalized_probability",
            ylabel="Normalized true-sum probability",
            ylim=(0.0, 1.05),
        )
        combine_vertical_figure_assets(
            [
                main_dir / "runtime_accuracy_tradeoff_biased_models_by_terms.png",
    ...
    plot_adaptive_topk_search_iterations(
        ...
        ylabel="Candidate path-mass cutoff",
    ...
    plot_adaptive_topk_search_iterations(
        ...
        ylabel="Mean retained raw posterior mass",
    ...
    root_readme_lines = [
        ...
        "",
        f"Figures live under {figure_root}",
    ...
    print(f"Saved visualization tables to: {vis_root}")
    print(f"Saved figure bundle to: {figure_root}")
