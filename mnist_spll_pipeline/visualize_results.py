        ...
        detailed_rows.append(
            {
                ...
                "cutoff_mode": str(run.get("cutoff_mode", "local")),
...
def plot_target_vs_achieved(summary_rows: List[Dict[str, Any]], output_path: Path) -> List[Dict[str, Any]]:
    ...
    model_rows = sorted(model_rows, key=lambda row: float(row.get("target_accuracy", 0.0)))
...
def run_visualization_stage(config: Dict[str, Any]) -> None:
    ...
    stage_message(3, 3, "Writing tables and separate local/global figure bundles")
    ...
    root_readme_lines = [
        ...
        "",
        f"Cutoff modes available: {', '.join(cutoff_modes)}",
        f"Terms shown: {', '.join(str(value) for value in term_counts)}",
        f"Cutoffs shown: {', '.join(pretty_threshold_label(label) for label in threshold_order)}",
        "",
        "Per-mode figures live under figures/<cutoff_mode>/...",
        "Per-mode tables live under tables/<cutoff_mode>/...",
    ]

    for cutoff_mode in cutoff_modes:
        mode_rows = [row for row in summary_by_terms if str(row.get("cutoff_mode", "local")) == cutoff_mode]
        if not mode_rows:
            continue
        ...
        overhead_rows = plot_overhead_exact_vs_zero(
            ...
            term_counts=term_counts,
            threshold_order=threshold_order,
            output_path=mode_appendix_dir / "overhead_exact_vs_zero_cutoff_by_terms.png",
        )
